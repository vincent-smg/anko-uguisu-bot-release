...
class LevelingSystem(commands.Cog):
        ...
        for uid, data in sorted_users:
            ...
            if member is None:
                try:
                    ...
                    print(f"[leaderboard] Could not fetch {uid}: {e}")
                    continue
    ...
    async def cmd_setpremium(self, interaction: discord.Interaction, username: str, grant: bool):
        ...
        if grant:
            ...
            from marriage import give_item, get_user_items, ITEM_POOL
            premium_item = next((i for i in ITEM_POOL if i["id"] == "deco_premium"), None)
            ...
            self.premium_users.discard(target_user.id)
            await interaction.followup.send(f"✅ Revoked Premium from **{target_user.name}**.", ephemeral=True)
        self.save_data()
